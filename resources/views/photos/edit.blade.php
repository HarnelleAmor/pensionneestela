                ...
                <div class="card"> 
                    <div class="card-body"> 
                        <div class="d-flex justify-content-between"> 
                            ...
                            <form id="deletePic{{ $photo->id }}" action="{{ route('photos.destroy', $photo->id) }}" method="POST" style="display: none;"> 
                                @csrf 
                                @method('DELETE') 
                            </form> 
                        </div>
                            ...
                            <div class="col-lg-6 col-md-7"> 
                                <form action="{{ route('photos.update', $photo->id) }}" method="POST"> 
                                    ...
                                    <div class="row align-items-center mb-3"> 
                                        ...
                                        <div class="col-auto"> 
                                            <textarea name="descr" id="descr" class="form-control @error('descr') is-invalid @enderror w-100" rows="1" required>{{ $photo->descr }}</textarea> 
                                        </div> 
                                        @error('descr') 
                                            <span class="text-danger">{{ $message }}</span> 
                                        @enderror 
                                    ...
                                    <div class="form-check"> 
                                        <input class="form-check-input" type="radio" name="is_archived" id="active" @checked(!$photo->is_archived)> 
                                        <label class="form-check-label" for="active"> 
                                            Active 
                                        </label> 
                                    </div> 
                                    <div class="form-check mb-3"> 
                                        <input class="form-check-input" type="radio" name="is_archived" id="archive" @checked($photo->is_archived)> 
