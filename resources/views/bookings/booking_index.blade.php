                        ...
                        <tbody>
                            @foreach ($bookings as $booking)
                                <tr>
                                    ...
                                    <td class="text-nowrap text-center">
                                        <i class="bi bi-moon-fill me-1"></i>{{ \Carbon\Carbon::parse($booking->checkin_date)->diffInDays(\Carbon\Carbon::parse($booking->checkout_date)) }}
                                    </td>
                                    <td class="text-nowrap">
                                        <ul>
                                            @forelse ($booking->services as $service)
                                                <li>
                                                    ...
                                                    <small data-bs-toggle="tooltip" data-bs-title="{{ $service->service->details }}">
                                                        Meal Service
                                                    </small>
                                                    @else
                                                    <small>
                                                        {{ $service->service->quantity . ' ' . $service->name }} (&#8369;{{ number_format($service->service_cost * $service->service->quantity, 2) }})
                                                    </small>
                                                    @endif
                                                </li>
                                            @empty
                                    ...
                                    <td class="text-nowrap text-center">&#8369;{{ number_format($booking->total_payment - $booking->outstanding_payment, 2) }}</td>
                                    <td class="text-nowrap text-center">&#8369;{{ number_format($booking->outstanding_payment, 2) }}</td>
                                    <td class="text-nowrap text-center">&#8369;{{ number_format($booking->total_payment, 2) }}</td>
                                    <td class="text-nowrap text-center">
                                        @if (is_null($booking->gcash_ref_no))
                                            <span class="text-muted fst-italic" data-bs-toggle="tooltip" data-bs-title="This person paid in cash.">Cash Transaction</span>
                                        @else
